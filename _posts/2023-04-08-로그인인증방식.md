...
```mermaid!
sequenceDiagram
클라이언트(A) ->> 서버: id, pwd 전송
서버 ->> 서버 : 유효성 검사
서버 ->> 클라이언트(A) : 인증 성공 + loginUser(A) 쿠키
클라이언트(A) ->> 서버 : 요청 + loginUser(A) 쿠키
서버 ->> 클라이언트(A) : 응답(A 정보)
```
...
```mermaid!
sequenceDiagram
브라우저(사용자) ->> server : 로그인 요청
server ->> db : 로그인 요청
db ->> server : 로그인 응답
server ->> session 저장소 : 회원정보 + session id 발급 요청
session 저장소 ->> server : session id 발급
server ->> 브라우저(사용자) : 응답 + session id (쿠키에 넣어서 전달)
```
...
```mermaid!
sequenceDiagram
브라우저(client A) ->> server : 요청 + cookie (session id)
server ->> session 저장소 : session id 인증 요청
session 저장소 ->> session 저장소 : 검증
session 저장소 ->> server : 유저 정보 획득
server ->> 브라우저(client A) : 응답
```
- 장점
...
```mermaid!
sequenceDiagram
브라우저(사용자) ->> 서버 : 사용자 로그인 요청
서버 ->> 회원 DB 서버 : 로그인 요청
회원 DB 서버 ->> 서버 : 로그인 응답
서버 ->> 서버 : Access Token 발급
서버 ->> 브라우저(사용자) : 응답 + Access Token (쿠키)
```

#### 발급받은 토큰을 이용한 요청 과정
```mermaid!
sequenceDiagram
브라우저(사용자) ->> 서버 : 요청 + Access Token(쿠키)
서버 ->> 서버 : Access Token 검증
서버 ->> 브라우저(사용자) : 응답
```
...
sequenceDiagram
브라우저(사용자) ->> 서버 : 요청 + Access Token(쿠키)
서버 ->> 서버 : Access Token 만료 확인
서버 ->> 브라우저(사용자) : Access Token 만료 신호
브라우저(사용자) ->> 서버 : Access Token 발급 요청(+ Access Token, Refresh Token)
서버 ->> 서버 : Refresh Token 검증 후, 새로운 Access Token 발급
서버 ->> 브라우저(사용자) : 응답 + 새로운 Access Token 발급
```
- 장점 
