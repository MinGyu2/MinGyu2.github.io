...
##### 로그인 인증 (`/sign_in_page/authentication` )
0. 먼저 세션 키가 존재하는지 확인하자. 존재하면 바로 메인페이지로 이동한다.
1.  post를 통해 요청이 들어오면 먼저 아이디와 비밀번호를 확인한다.
2. `LoginAuthentication`클래스의 인스턴스를 만들어준다. 인증 클래스의 `userAuthenticatoin` 함수에 유저 이름과 패스워드를 전송해준다.
3. 인증이 성공하면 `User` 정보를 얻을 수 있다. 하지만 인증에 실패하면 `null` 값을 받게된다.
4. 즉 null 값을 받아 로그인에 실패하면 `/sign_in_page` 다시 로그인 페이지로 리다이렉트 요청을 한다.
5. `User` 정보를 받으면 로그인에 성공했다는 것이다.  따라서 `User` 오브젝트를 세션에 저장하여 준다. 세션 정보는 session id를 통해 가져올 수 있다. 
6. 확인해 보니 session id는 자동으로 쿠키에 저장 된다.
7. 로그인에 성공하고 session id 도 생성되었다. 마지막으로 메인 페이지로 리다이렉션 해준다.<br>
